	...
	toolSettings.exportData.keyMap =  {					// not required
        ...
        toolSettings.quickZoomTools.tools = [
                {id: 1,                                                 // ids must be unique
                 label: 'Pick a site',
                 layer: 'massgis:AFREEMAN.SITES_TEST_POLY',
                 valueField: 'SITE_ID',
                 sortBy : 'SITE_ID', // optional, fieldname
                 sortOrder : 'A', // optional, acceptable values = A, D
                 additionalFields : 'SITE_ID',  // optional will be requested, often used by other restriction filters
                 resetOnMove: false
                }
        ];
	...
	toolSettings.editTool.layers = [
		{
			featureType: "AFREEMAN.BLDGS_TEST_POLY",
			layerTitle : "Buildings Test Poly", // must match whatever is found in folderset for this layer.
			fields : [
				{
					...
					triggerAction: 'all',
					store_remote : {
						layer : 'massgis:AFREEMAN.BLDGS_NAMES',
						valueField : 'NAME',
						sortBy : 'NAME', 
						sortOrder : 'D', 
						restrict : {
							type : 'url' // or 'static', or 'url'
							,value : 'site_id'// qzID (or string, or url param_name
							,def_val : '10B'// only meaningful if type='url'
							,restrictedValueField: 'SITE_ID'  // name of the field in this layer
							,restrictedSourceField: 'SITE_ID' // name of the field in the layer we're restricting based on			
						}
					}
				}
